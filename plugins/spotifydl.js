...
const handler = async (m, { bot, text, reply2 }) => {
   if (!text) {
      return reply2("Masukkan nama lagu atau artis untuk dicari!");
   }
...
`;
   ...
   if (audio.url) {
      bot.sendMessage(m.chat, { audio: { url: audio.url }, mimetype: "audio/mpeg" }, { quoted: m });
   } else {
      reply2("Tidak ada audio yang tersedia untuk diunduh.");
   }
};


handler.command = ["play", "spotify"]
handler.limit = false;
